...
export function SideBar({ sideBarItemList }: SideBarProps) {
  ...
  const renderSideBarItemList = () =>
      ...
      const isActive =
        ...
        (item.linkPath === "/Patterns" && (location.pathname === "/Patterns" || location.pathname === "/Patterns/"));
  ...
  return (
    <div>
      <div
        ...
        className="hs-overlay [--auto-close:lg] hs-overlay-open:translate-x-0 -translate-x-full transition-all duration-300 transform hidden fixed lg:relative top-0 start-0 bottom-0 h-screen z-[60] bg-solid-base w-72 md:w-64 overflow-y-auto overflow-x-hidden lg:block lg:translate-x-0 [&::-webkit-scrollbar-track]:bg-gray-100 [&::-webkit-scrollbar-thumb]:bg-gray-300 dark:[&::-webkit-scrollbar-track]:bg-neutral-700 dark:[&::-webkit-scrollbar-thumb]:bg-neutral-500 dark:bg-solid-dark-base dark:border-solid-dark-base border-r border-gray-200 dark:border-gray-dark-200"
        role="dialog"
        aria-label="Sidebar"
      >
        <nav className="hs-accordion-group w-full flex flex-col py-6" data-hs-accordion-always-open>
